...
public class CarAIControl : MonoBehaviour
{
    ...
    {
        DisableBraking,                 // the car simply accelerates at full throttle all the time.
        DirectionOffset,  // the car will brake according to the upcoming change in direction of the target. Useful for route-based AI, slowing for corners.
        DistanceToTarget,             // the car will brake as it approaches its target, regardless of the target's direction. Useful if you want the car to
    ...
    [SerializeField] private float accelerationSensitivity = 0.04f;                                // How sensitively the AI uses the accelerator to reach the current desired speed
    ...
    [SerializeField][Range(0, 1)] private float accelerationWanderAmount = 0.1f;                  // how much the cars acceleration will wander
    [SerializeField] private float accelerationWanderSpeed = 0.1f;                                 // how fast the cars acceleration wandering will fluctuate
    [SerializeField] private BrakeCondition brakeCondition = BrakeCondition.DistanceToTarget; // what should the AI consider when accelerating/braking?
    ...
    private float PerlinOffset;             // A random value for the car to base its wander on (so that AI cars don't all wander in the same pattern)

    // ── Reference updated to your BMWCarController ───────────────────────────
    private BMWCarController carController; // Reference to actual car controller we are controlling
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            if (m_Rigidbody.linearVelocity.magnitude > carController.MaxSpeed * 0.1f)
            {
            ...
            {
                case BrakeCondition.DirectionOffset:
                    {
                ...
                case BrakeCondition.DistanceToTarget:
                    {
                ...
                case BrakeCondition.DisableBraking:
                    break;
            }
            ...
            if (Time.time < avoidOtherCarTime)
            {
                ...
                offsetTargetPos += m_Target.right * avoidPathOffset;
            }
            else
            {
                ...
                offsetTargetPos += m_Target.right *
                                   (Mathf.PerlinNoise(Time.time * lateralWanderSpeed, PerlinOffset) * 2 - 1) *
            ...
            accel *= (1 - accelerationWanderAmount) +
                     (Mathf.PerlinNoise(Time.time * accelerationWanderSpeed, PerlinOffset) * accelerationWanderAmount);
